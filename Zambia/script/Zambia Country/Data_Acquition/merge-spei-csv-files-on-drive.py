...
# CONFIGURATION
# -------------------------------
DRIVE_FOLDER_ID = '1-y6n3CUPGbu6xQmCRU9qkRmNJCiHGH4r'  # e.g., '1abc123...' from the folder URL
LOCAL_TEMP_DIR = './temp_csvs'            # temporary folder to download files
MASTER_CSV_NAME = 'master_southern_province_data.csv'
...
def merge_csv_files(file_paths):
    ...
    return master
        # If the file contains NDVI/EVI, rename columns for clarity
        #if 'ndvi' in df.columns and 'evi' in df.columns:
        #    df = df.rename(columns={'ndvi': 'NDVI', 'evi': 'EVI'})
        
        # If the file contains LST_Celsius
        #if 'lst_celsius' in df.columns:
        #    df = df.rename(columns={'lst_celsius': 'LST_C'})
        
        # If the file contains precipitation and PET
        #if 'precip_mm' in df.columns and 'pet_mm' in df.columns:
        #    pass  # keep as is
        #
        ## Ensure key columns exist and are of correct type
        #for col in ['district', 'year', 'month', 'day', 'date']:
        #    if col not in df.columns:
        #        # Try to infer from other names
        #        if col == 'district' and 'adm_name' in df.columns:
        #            df = df.rename(columns={'adm_name': 'district'})
        #        elif col == 'district' and 'name' in df.columns:
    ...
    #master = all_dfs[0]
    #for df in all_dfs[1:]:
    #    # Determine how to merge: if they share only district and date keys, join
    #    common_keys = ['district', 'year', 'month', 'day', 'date']
    #    if all(k in master.columns and k in df.columns for k in common_keys):
    #        # Horizontal merge (join) on all common keys
    #        master = pd.merge(master, df, on=common_keys, how='outer')
    #    else:
    #        # Otherwise, just concatenate rows (e.g., different years of same variable)
